...
P_eP_a = 2.0
phi_e = 0.0 # deg
n = 18 # number of characteristics in the centered expansion fan
        ...
        if i == n+1:
            if j == 0:
                ...
                x[count] = (y[out_count_p[j]] - y[count-1] + slopes_negative[count_neg]*x[count-1]
                            - slopes_positive[count_plus_char[j]]*x[out_count_p[j]]) / (slopes_negative[count_neg] - slopes_positive[count_plus_char[j]])
    ...
    for j in range(i):
        if i == n:
            if j == 0: # This point is already determined in the previous loop
                ...
                x[count_out_3[j]] = (y[count_out_3[j]-1] - y[count_out_3[j-1]] + slopes_positive[count_slopos_3[j-1]]*x[count_out_3[j-1]] - slopes_negative[count_out_3[j]-2-j]*x[count_out_3[j]-1])\
                           /(slopes_positive[count_slopos_3[j-1]]-slopes_negative[count_out_3[j]-2-j])
                y[count_out_3[j]] = slopes_positive[count_slopos_3[j-1]]*(x[count_out_3[j]]-x[count_out_3[j-1]]) + y[count_out_3[j-1]]
                if plot_chars == True:
            ...
            if j == 0:
                if i == 1:
                    ...
                    plt.fill((x[count_regions-n], x[count_regions-n+1], x[count_regions+i-n], x[count_regions-n]),
                         (y[count_regions - n], y[count_regions - n + 1], y[count_regions+i-n], y[count_regions - n]), color=weight_color[count_regions])
            elif j == i-1:
                ...
                plt.fill((x[count_regions - n], x[count_regions - n + 1], x[count_regions + i - n], x[count_regions + i - n - 1], x[count_regions - n]),
                         (y[count_regions - n], y[count_regions - n + 1], y[count_regions + i - n], y[count_regions + i - n - 1], y[count_regions - n]),
                         color=weight_color[count_regions])
            count_regions += 1
...
while x_s[count] < x[-1] and i>1:
    x_s[count] = (y[count_finals[n-i+1]] - y_s[count - 1] + slopes_phi[count_finals[n-i+1]] * x_s[count - 1] - slopes_negative[count_finals[n-i+1]+(i-n-2)] * x[count_finals[n-i+1]]) \
